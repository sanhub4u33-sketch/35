...
const UserDashboard = () => {
  ...
  const analytics = useMemo(() => {
    ...
    const attendanceRate = daysInCurrentMonth > 0 ? Math.round((thisMonthDays / daysInCurrentMonth) * 100) : 0;
                ...
                <tbody>
                  {memberDues.map((due) => (
                    <tr key={due.id} className="border-b border-border">
                      ...
                      <td className="p-2 sm:p-3 text-sm hidden sm:table-cell">{due.receiptNumber || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
